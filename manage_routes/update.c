#ifndef UPDATE_ROUTES_SEEN
#define UPDATE_ROUTES_SEEN
...
int update_availability(int route_num, int availability){
    ...
    sprintf(query_update, "UPDATE `Route` SET `availability`=%d", availability);
...
int update_stops(
    ...
    // first delete the previous route stop entries
    char query_delete[100];
    sprintf(query_delete, "DELETE FROM `Route-Stop` WHERE `route_number`=%d", route_num);
    if(mysql_query(con, query_delete)){
        printf("An internal error occured. Please try again later\n");
        mysql_rollback(con);
        return -1;
    }
    char query_route_insert[query_length]; // 49
    
    int route_key = route_num; // 52
    ...
    mysql_commit(con);
    printf("SUCCESSFUL UPDATION OF ROUTE\n");
    end_transaction(con);
    return 1;
} 
